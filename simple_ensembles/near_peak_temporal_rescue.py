# flake8: noqa
"""Generated Shinka method snapshot: near_peak_temporal_rescue.
...
TEMPORAL_EARLY_PEAK_MARGIN = 0.075
TEMPORAL_EARLY_MIN_SUPPORT = 0.22
...
def _prediction_candidates(prediction_data_dir: Path, members: list[EnsembleMember], threshold: float) -> list[dict[str, Any]]:
    ...
    for tile_id in _prediction_tile_ids(prediction_data_dir):
        ...
        near_peak = (
            (probability_stack >= threshold)
            & (probability_stack >= (max_prob[None, :, :] - TEMPORAL_EARLY_PEAK_MARGIN))
            & (max_prob[None, :, :] >= threshold)
        )
        any_near_peak = np.any(near_peak, axis=0)
        first_near_peak_index = np.argmax(near_peak, axis=0)
        ...
        for year_index, (year, probability_grid, prior_grid, temporal_support, profile, model_names) in enumerate(per_year):
            ...
            early_peak_rescue = (
                any_near_peak
                & (first_near_peak_index == year_index)
                & (temporal_support >= TEMPORAL_EARLY_MIN_SUPPORT)
                & (probability_grid >= threshold)
            )

            candidate_probability = np.where(first_change | strong_absolute | argmax_rescue | prior_rescue | early_peak_rescue, probability_grid, 0.0)
