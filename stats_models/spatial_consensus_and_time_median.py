"""Promoted statistical model: spatial_consensus_and_time_median (from gen_173).
...
def predict_deforestation_probability(features: ArrayMap) -> np.ndarray:
    ...
    adaptive_canopy = np.clip(
        0.27 * ad_ndvi + 0.33 * ad_nbr + 0.17 * ad_ndmi + 0.10 * ad_evi + 0.13 * ad_bsi,
        0.0,
        1.0,
    )
    ...
    probability = np.where(
        optical_only_strong, np.maximum(probability, 0.54), probability
    )
    probability = np.where(
    ...
    probability = np.where(
        ultra_consistent, np.maximum(probability, 0.680), probability
    )
    ...
    weak_single_channel = (
        (evidence_count <= 1.0) & (persistent_loss < 0.42) & (canopy_loss < 0.38)
    )
    isolated_weak_alarm = (
        (local_support <= 0.11)
    ...
    return np.clip(probability, 0.0, 1.0).astype(np.float32) * forest_2020.astype(
        np.float32
    )
